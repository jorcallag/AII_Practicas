...
def extraer_juegos():
    ...
    for p in range(1,PAGINAS+1):
        ...
        for i in l:
            ...
            if t :#tienen alguna/s de las características adicionales
                temcom = t.tbody
                tematica = temcom.find("td",attrs={"data-th":"Temática"})
                if tematica:
                ...
                jugadores = temcom.find("td",attrs={"data-th":"Núm. jugadores"})
                if jugadores:
                    jugadores = jugadores.string.strip()
                else:
                    jugadores = "Desconocida"
            else: #no tienen características adicionales
                tematica = "Desconocida"
                complejidad = "Desconocida"
                jugadores="Desconocida"
            ...
            if d :#si hay detalles
                detalles = d.find("div",class_="product attribute description").find("div",class_="value")
                if detalles.div:
...
def imprimir_lista(cursor):
    ...
    for row in cursor:
        ...
        lb.insert(END,"    Temáticas: "+ row['tematicas'])
...
def almacenar_datos():
    ...
    schem = Schema(titulo=TEXT(stored=True), precio=NUMERIC(stored=True,numtype=float), tematicas=KEYWORD(stored=True,commas=True,lowercase=True), complejidad=ID(stored=True), jugadores=KEYWORD(stored=True,commas=True), detalles=TEXT)
...
def buscar_tematicas():
    ...
    l = Label(v, text="Seleccione temática a buscar:")
    l.pack(side=LEFT)
...
def buscar_precio():
    ...
    v = Toplevel()
    v.title("Búsqueda por Precio")
    l = Label(v, text="Introduzca el precio máximo:")
    l.pack(side=LEFT)
    en = Entry(v)
    en.bind("<Return>", mostrar_lista)
    en.pack(side=LEFT)
